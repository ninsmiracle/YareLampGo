...
| 链路 | 触发方式 | 组件 |
| --- | --- | --- |
| **唤醒词链路** | 持续监听麦克风，命中关键词（默认 `hey_jarvis`）后激活 | `openwakeword` + Xiaomi LiveKit Agent SDK + LiveKit Server |
...
可选依赖一并装好（`uv sync --extra voice` 或在 `pyproject.toml` 已声明的 optional `voice` 组中自动随 `xiaomi-livekit-agent-sdk` 一起安装）。
