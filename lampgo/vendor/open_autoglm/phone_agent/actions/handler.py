...
from lampgo.vendor.open_autoglm.phone_agent.config.timing import TIMING_CONFIG
from lampgo.vendor.open_autoglm.phone_agent.device_factory import get_device_factory
...
class ActionHandler:
    ...
    def _send_keyevent(self, keycode: str) -> None:
        ...
        from lampgo.vendor.open_autoglm.phone_agent.device_factory import DeviceType, get_device_factory
        from lampgo.vendor.open_autoglm.phone_agent.hdc.connection import _run_hdc_command
...
def _normalize_action_response(response: str) -> str:
    """Strip wrapper markup that some OpenAI-compatible models include."""
    response = response.strip()

    if "<answer>" in response:
        response = response.split("<answer>", 1)[1]
    if "</answer>" in response:
        response = response.split("</answer>", 1)[0]
    ...
    for marker in ("do(action=", "finish(message="):
        if marker in response and not response.startswith(marker):
