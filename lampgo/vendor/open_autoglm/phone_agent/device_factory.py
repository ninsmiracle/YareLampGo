...
class DeviceFactory:
    ...
    def module(self):
        ...
        if self._module is None:
            if self.device_type == DeviceType.ADB:
                from lampgo.vendor.open_autoglm.phone_agent import adb

                self._module = adb
            elif self.device_type == DeviceType.HDC:
                from lampgo.vendor.open_autoglm.phone_agent import hdc
    ...
    def get_connection_class(self):
        ...
        if self.device_type == DeviceType.ADB:
            from lampgo.vendor.open_autoglm.phone_agent.adb import ADBConnection

            return ADBConnection
        elif self.device_type == DeviceType.HDC:
            from lampgo.vendor.open_autoglm.phone_agent.hdc import HDCConnection
