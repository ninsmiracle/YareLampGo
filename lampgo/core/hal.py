"""Hardware Abstraction Layer — wraps lerobot FeetechMotorsBus.

Adapted from lelamp_runtime LeLampFollower. Stripped of camera logic and
built-in safety clipping (that responsibility now lives in SafetyKernel).
"""
...
class HardwareAbstraction:
    ...
    def calibrate(self) -> None:
        """Interactive calibration flow — adapted from LeLampFollower.calibrate()."""
        if self._bus is None:
    ...
    def _configure_motors(self) -> None:
        """Apply PID and operating mode settings. Adapted from LeLampFollower.configure()."""
        if self._bus is None:
            return
