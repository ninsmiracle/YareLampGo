...
class PhoneAgentConfig(BaseModel):
    ...
    direct_control_enabled: bool = Field(
        default=True,
        description="Use deterministic ADB commands for simple phone_task requests before falling back to the GUI agent.",
    )
...
def _apply_env_overrides(config: LampgoConfig, *, track: bool = False) -> list[str]:
    ...
    env_map = {
        ...
        "LAMPGO_PHONE_DIRECT_CONTROL_ENABLED": ("phone_agent", "direct_control_enabled"),
